...
if __name__ == "__main__":
    ...
    quadrupleTotal, quadrupleList, tripleDict, _ = load_quadruples('./data/icews14/', 'train2id.txt', 'train_tem.npy', 'valid2id.txt', 'valid_tem.npy')
    config = Config()
    ...
    # lstm_embeddings = model.lstm.weight.data.cpu().numpy()
    lstm_embeddings = model.lstm.cpu()
    L1_flag = model.L1_flag
    filter = model.filter

    hit10Test, meanrankTest = evaluation(testList, tripleDict, ent_embeddings, rel_embeddings, tem_embeddings, lstm_embeddings, L1_flag, filter, head=0)
